...
│   └── vercel-deploy.md   · Vercel 배포는 사용자 확인 후 진행
├── skills/                ← 실행 절차(규칙의 "어떻게", 호출형)
│   ├── blog-post/SKILL.md    · 네이버 블로그 → 홈페이지 게시글 변환 절차 (/blog-post)
│   └── deploy-relay/SKILL.md · Cusdis→Slack relay Vercel 배포 절차 (/deploy-relay, 실행 전 확인)
└── hooks/                 ← 자동 검증/강제
...
- **Vercel 배포**: `vercel`/`vercel --prod` 등은 실행 전 사용자에게 물어보고 승인받은 뒤 진행. → [wiki/vercel-deploy.md](wiki/vercel-deploy.md)
