...
2. **크롤링** (헤드리스 Chrome, 모바일 UA):
   ...
   - 첨부 파일도 확인: `.se-module-fileList a`/`[data-linkdata]`의 `link`/"파일 다운로드"(`download.blog.naver.com`) 링크 (wiki 2'절).

3. **이미지·첨부 저장**:
   - 이미지: Referer 헤더로 다운로드 → **`static/img/blog/<slug>-<n>.<ext>` 파일로 저장**. base64 인라인 금지(성능). 본문은 `![alt](/img/blog/...)` 경로 참조.
   - 첨부 파일(자료 나눔 PDF 등)이 있으면 **`static/downloads/<slug>.<ext>`로 저장**하고 본문에 다운로드 버튼(`<a href download>` 주황 pill)을 넣는다 (wiki 2'절).

4. **본문 정리·작성** (wiki 4~6절):
   - 푸터·연락처·링크카드·지도위젯·영상 잔여물·"파일 다운로드" UI 텍스트·해시태그·상단 반복 인사말 제거(단, 실제 첨부 파일은 3단계처럼 저장·버튼화).
