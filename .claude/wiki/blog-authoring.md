...
- ⚠️ **볼드 닫힘 깨짐 규칙**: 볼드가 **`)`·`]`·따옴표(`'"’”`)로 끝난 뒤 바로 한글**이 붙으면 닫는 `**`를 인식 못 해 `**`가 그대로 노출된다(CommonMark right-flanking). 예: `**'영유아 검진'**이라는`, `**PECS(펙스)**를` → 깨짐.
  - **해법(우선): 닫는 `**` 뒤에 한 칸 띄운다** → `**'영유아 검진'** 이라는`, `**PECS(펙스)** 를`. (괄호·따옴표를 볼드 밖으로 빼지 말 것 — 사용자 지정 방식)
  - 한글로 둘러싸인 볼드(`**루피**를`)는 정상이니 손대지 않는다. 발화/인용 강조는 `<mark>"..."</mark>` 도 가능.
  - 작성 후 점검: `grep -rnoP '[)\]”"’'"'"']\*\*[가-힣]' blog/*.mdx` (여는 `"**루` 류는 오탐).
