...
- 다운로드 시 Referer(`https://m.blog.naver.com/joylangcenter`) 헤더를 넣는다.
- ⚠️ **성능 규칙(중요)**: 이미지는 **`static/img/blog/` 에 파일로 저장**하고 본문에서 **경로로 참조**한다 (`![alt](/img/blog/<slug>-<n>.<ext>)`). **base64 data URI 인라인 금지.**
  - 이유: base64를 본문에 넣으면 글당 ~1MB씩 커지고, 보드(`BlogCards`)가 import하는 블로그 아카이브 JSON이 통째로 비대해져(예: 33글에 36MB) **홈·커뮤니티 전 페이지 로딩이 급격히 느려진다.** 파일로 빼면 아카이브 JSON이 수백 KB로 줄고 이미지는 캐시·지연 로딩된다.
  - 파일명: `static/img/blog/<slug>-<n>.<ext>` (ext: png/jpg/gif). 확장자는 원본 mime에 맞춘다.
...
- 단, 제목·리스트·표·이미지·frontmatter 안에서는 개행하지 않는다. 인용구(`>`)는 `>` 접두어를 유지한 채 개행.
